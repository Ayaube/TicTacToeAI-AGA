...
void Plateau::prochainMove(GameMove &myMove, GameMove &lastMove){
    ...
    // Si le sous-plateau cible n'est pas condamn�, jouer dedans
    if (!estCondamne(cibleL, cibleC)) {
        ...
        // 2. Si on a trouv� un blocage, on le joue
        if (found) return;

        // 3. Sinon, choisir strat�giquement
        ...
        for (auto& pos : ordre) {
            ...
            if (getCase(r, c) == 0) {
                ...
                // Bonus si on envoie l'IA dans un sous-plateau condamn� (elle joue libre)
                // Malus si on l'envoie dans un sous-plateau qu'elle domine
                if (m_etat[envoyeL][envoyeC] != 0) {
                    score -= 10; // �viter de lui donner le choix libre
                } else {
                    // Compter nos pi�ces vs les siennes dans le sous-plateau cible
                    int nosP = 0, sesP = 0;
                    for (int i = 0; i < 3; i++)
                        for (int j = 0; j < 3; j++) {
                    ...
                    score = nosP - sesP; // on pr�f�re l'envoyer o� on domine
                }
    ...
    // Si condamn� ou plein, jouer n'importe o�
    if (!found) {
        for (int r = 0; r < 9; r++) {
            for (int c = 0; c < 9; c++) {
                if (getCase(r, c) == 0 && !estCondamne(r, c)) {
